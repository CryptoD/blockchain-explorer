...
|--------|------------------|-------------------------|
| **Content-Security-Policy (CSP)** | `default-src 'self'`; `script-src` `'self'` + `https://cdn.jsdelivr.net` (+ optional **`CDN_BASE_URL`** host when set—must match stamped HTML); `style-src` `'self'` + `'unsafe-inline'` (+ same CDN host); `img-src` `'self' data: https:` (+ same CDN host); `connect-src 'self'`; `frame-ancestors 'none'`; `base-uri 'self'`; `form-action 'self'`; `object-src 'none'` | Reduces XSS impact and clickjacking (`frame-ancestors`). Style **`unsafe-inline`** may remain for Tailwind/dynamic markup; see roadmap task **39**. CDN: task **49**, [CDN_STATIC_ASSETS.md](CDN_STATIC_ASSETS.md). |
