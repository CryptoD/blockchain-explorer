...
| **Key layout** | Central prefixes and builders in [`internal/repos/keys.go`](../internal/repos/keys.go) (`UserKey`, `PortfolioKey`, `SessionKey`, …). **API key** metadata and indexes use **`apikey:v1:*`** in [`internal/repos/apikey.go`](../internal/repos/apikey.go) (plaintext tokens are never stored—only hashes). |
