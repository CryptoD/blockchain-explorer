...
**Inventory note:** The table below includes packages added through roadmap work through task **72** (e.g. [`internal/idempotency`](../internal/idempotency/), [`internal/featureflags`](../internal/featureflags/), [`internal/outboundbreaker`](../internal/outboundbreaker/), [`internal/retrybudget`](../internal/retrybudget/), email queue + dead-letter behavior in [`internal/email`](../internal/email/email.go)). **HTTP contract:** hand-maintained [`openapi.yaml`](../openapi.yaml) describes `/api/v1` (see [`internal/server/routes.go`](../internal/server/routes.go)); release notes live in [`CHANGELOG.md`](../CHANGELOG.md) with `openapi.yaml` **`info.version`** aligned on tagged releases; partners can import [`postman/blockchain-explorer.postman_collection.json`](../postman/blockchain-explorer.postman_collection.json); machine authentication in [`API_KEYS.md`](API_KEYS.md) (`Authorization: Bearer bkx_*`); versioning and deprecation (**notice, Sunset / Deprecation headers**) are policy in [`API_VERSIONING.md`](API_VERSIONING.md); JSON errors use [`apiutil.ErrorEnvelopeJSON`](../internal/apiutil/error_envelope.go); paginated lists use [`apiutil.ListPagination`](../internal/apiutil/pagination.go); CI validates the spec via [`scripts/check-openapi.sh`](../scripts/check-openapi.sh). Extend the table when new `internal/` boundaries appear.
