...
**Inventory note:** The table below includes packages added through roadmap work through task **69** (e.g. [`internal/idempotency`](../internal/idempotency/), [`internal/featureflags`](../internal/featureflags/), [`internal/outboundbreaker`](../internal/outboundbreaker/), [`internal/retrybudget`](../internal/retrybudget/), email queue + dead-letter behavior in [`internal/email`](../internal/email/email.go)). **HTTP contract:** hand-maintained [`openapi.yaml`](../openapi.yaml) describes `/api/v1` (see [`internal/server/routes.go`](../internal/server/routes.go)); partners can import [`postman/blockchain-explorer.postman_collection.json`](../postman/blockchain-explorer.postman_collection.json) (regenerate via [`scripts/gen-postman-collection.py`](../scripts/gen-postman-collection.py)); versioning and deprecation (**notice, Sunset / Deprecation headers**) are policy in [`API_VERSIONING.md`](API_VERSIONING.md); **PUT/PATCH idempotency and portfolio concurrency** in [`REST_WRITE_SEMANTICS.md`](REST_WRITE_SEMANTICS.md); JSON errors use [`apiutil.ErrorEnvelopeJSON`](../internal/apiutil/error_envelope.go) (**`code`, `message`, `correlation_id`, `timestamp`**); paginated lists use [`apiutil.ListPagination`](../internal/apiutil/pagination.go) (**`total`, `page`, `page_size`, `has_more`**); CI validates the spec via [`scripts/check-openapi.sh`](../scripts/check-openapi.sh). Extend the table when new `internal/` boundaries appear.
...
| [`internal/apiutil`](../internal/apiutil/) | Evolving | Pagination/sort (`ParsePagination`, …), [`ListPagination`](../internal/apiutil/pagination.go), JSON errors ([`ErrorEnvelopeJSON`](../internal/apiutil/error_envelope.go)) |
